...
class AdventureSetCommands(AdventureMixin):
    ...
    async def adventureset_locks(self, ctx: commands.Context):
        """[Admin] Reset Overkill RPG locks."""
    ...
    async def easymode(self, ctx: commands.Context):
        """[Owner] Set whether or not Overkill RPG will be in easy mode.
        ...
        await smart_embed(
            ctx, _("Overkill RPG easy mode is now {}.").format(bold(_("Enabled") if not toggle else _("Disabled")))
        )
    ...
    async def showsettings(self, ctx: commands.Context):
        ...
        msg += _("[Overkill RPG message style]:              {adventure_in_embed}\n").format(
